...
def check_for_collision():
    balloon_bbox = canvas.bbox(balloon)
    cactus_bbox = canvas.bbox(cactus)
    overlap = not (balloon_bbox[2] < cactus_bbox[0] or   # balloon is to the left of cactus
                   balloon_bbox[0] > cactus_bbox[2] or   # balloon is to the right of cactus
                   balloon_bbox[3] < cactus_bbox[1] or   # balloon is above cactus
                   balloon_bbox[1] > cactus_bbox[3])     # balloon is below cactus
    if overlap:
        return True
